...
// Firebase removed
interface User {}
import { Zap, Battery, Lightbulb, Minus, Plus, Play, Pause, RotateCcw, Save, Target, Activity } from 'lucide-react';
        ...
        <div className="flex justify-center mb-8">
          <div className="bg-black/40 backdrop-blur-lg rounded-2xl border border-white/10 p-2">
            <div className="flex space-x-2">
              {[
                { id: 'circuits', label: 'Circuit Analysis', icon: Zap },
                { id: 'projectile', label: 'Projectile Motion', icon: Target },
                { id: 'waves', label: 'Traveling Wave', icon: Plus },
                { id: 'pendulum', label: 'Pendulum', icon: Activity },
                { id: 'library', label: 'Sim Library', icon: Plus }
              ].map(exp => {
                const Icon = exp.icon;
                return (
                  <button
        ...
        {activeExperiment === 'library' && (
          <div className="space-y-8">
            <div className="bg-black/40 backdrop-blur-lg rounded-2xl border border-white/10 p-6">
              ...
              <div className="flex flex-col xl:flex-row xl:items-center xl:space-x-2 space-y-2 xl:space-y-0 mb-4">
                <input
                  value={simSearch}
                  onChange={e => setSimSearch(e.target.value)}
                  placeholder="Search simulations..."
                  className="flex-1 bg-gray-800/70 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-600"
                />
                <button
                ...
                <select
                  ...
                  className="flex-1 xl:max-w-xs bg-gray-800/70 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-600"
                >
                ...
                <select
                  ...
                  className="flex-1 md:max-w-xs bg-gray-800/70 border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-600"
                >
                  {SIMULATIONS.map(sim => (
              ...
              <div className="flex items-center justify-between text-xs text-gray-400 mb-2">
                ...
                <button
                  ...
                  className="ml-4 px-3 py-1 rounded bg-white/5 hover:bg-white/10 border border-white/10 text-gray-300"
                >{simExpand ? 'Collapse' : 'Expand All'}</button>
              </div>
              <div className={`grid md:grid-cols-4 gap-3 ${simExpand ? 'max-h-none' : 'max-h-72 overflow-y-auto pr-2'}`}>
                {SIMULATIONS.filter(sim => {
                  ...
                  <button
                    ...
                    className={`text-left px-3 py-2 rounded-lg text-sm border transition-colors ${
                      selectedSimId === sim.id ? 'bg-blue-600 border-blue-500 text-white' : 'bg-white/5 border-white/10 text-gray-300 hover:bg-white/10'
                    }`}
                  >
              ...
              <div className="mt-8">
                ...
                <div className="bg-gray-900/40 rounded-lg border border-white/10 p-4 max-h-64 overflow-auto text-xs leading-relaxed font-mono">
                  {SIMULATIONS.map(s => (
                    <div key={`list-${s.id}`} className="text-gray-300"><span className="text-blue-400">{s.id}</span> – {s.name}</div>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-4 mb-6">
                <div className="grid grid-cols-2 gap-2 mb-2">
                  ...
                  <button
                    ...
                    className="w-full p-2 bg-blue-800 hover:bg-blue-900 text-white rounded-lg transition-colors flex items-center justify-center space-x-2 text-xs"
                  >
                    <span>+ Ammeter</span>
                  </button>
                  <button
                    onClick={() => addCircuitComponent('voltmeter')}
                    className="w-full p-2 bg-green-800 hover:bg-green-900 text-white rounded-lg transition-colors flex items-center justify-center space-x-2 text-xs"
                  >
                    <span>+ Voltmeter</span>
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className={`px-3 py-1 rounded ${batteryMode === 'series' ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-300'}`}
                    >Series</button>
                    <button
                      onClick={() => setBatteryMode('parallel')}
                      className={`px-3 py-1 rounded ${batteryMode === 'parallel' ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-300'}`}
                    >Parallel</button>
                  </div>
                </div>
                {/* Bulb Mode Toggle */}
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className={`px-3 py-1 rounded ${bulbMode === 'series' ? 'bg-yellow-600 text-white' : 'bg-gray-700 text-gray-300'}`}
                    >Series</button>
                    <button
                      onClick={() => setBulbMode('parallel')}
                      className={`px-3 py-1 rounded ${bulbMode === 'parallel' ? 'bg-yellow-600 text-white' : 'bg-gray-700 text-gray-300'}`}
                    >Parallel</button>
                  </div>
                </div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className={`px-3 py-1 rounded ${resistorMode === 'series' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300'}`}
                    >Series</button>
                    <button
                      onClick={() => setResistorMode('parallel')}
                      className={`px-3 py-1 rounded ${resistorMode === 'parallel' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300'}`}
                    >Parallel</button>
                  </div>
                </div>
                <button
                ...
                <button
                  ...
                  className="w-full p-3 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg transition-colors flex items-center justify-center space-x-2"
                >
                  <Lightbulb size={18} />
            ...
            <div className="bg-black/40 backdrop-blur-lg rounded-2xl border border-white/10 p-6">
              ...
              <div className="bg-gray-900/50 rounded-lg p-4 mb-6 h-64 relative overflow-hidden">
                ...
                <div className="relative h-full">
                  <div className="absolute left-12 top-16 w-8 h-8 bg-green-500 rounded border-2 border-white flex items-center justify-center">
                    <Battery size={16} className="text-white" />
                  </div>
                  
                  <div className="absolute left-32 top-8 w-8 h-8 bg-blue-500 rounded border-2 border-white flex items-center justify-center">
                    <Minus size={16} className="text-white" />
                  </div>
                  
                  <div className="absolute left-52 top-16 w-8 h-8 bg-yellow-500 rounded-full border-2 border-white flex items-center justify-center">
                    <Lightbulb 
                      size={16} 
            ...
            <div className="bg-black/40 backdrop-blur-lg rounded-2xl border border-white/10 p-6">
              ...
              <div className="space-y-6">
                <div className="bg-gradient-to-r from-green-600/20 to-green-800/20 rounded-lg p-4 border border-green-600/30">
                  <div className="text-green-300 text-sm font-medium mb-1">Voltage (V)</div>
                  <div className="text-2xl font-bold text-white">{voltage.toFixed(1)} V</div>
                </div>
                
                <div className="bg-gradient-to-r from-blue-600/20 to-blue-800/20 rounded-lg p-4 border border-blue-600/30">
                  <div className="text-blue-300 text-sm font-medium mb-1">Current (I)</div>
                  <div className="text-2xl font-bold text-white">{current.toFixed(3)} A</div>
                </div>
                
                <div className="bg-gradient-to-r from-purple-600/20 to-purple-800/20 rounded-lg p-4 border border-purple-600/30">
                  <div className="text-purple-300 text-sm font-medium mb-1">Resistance (R)</div>
                  <div className="text-2xl font-bold text-white">{resistance.toFixed(0)} Ω</div>
                </div>
                
                <div className="bg-gradient-to-r from-orange-600/20 to-orange-800/20 rounded-lg p-4 border border-orange-600/30">
                  <div className="text-orange-300 text-sm font-medium mb-1">Power (P)</div>
              ...
              <div className="mt-6 p-4 bg-gray-800/50 rounded-lg">
                ...
                <div className="text-sm text-gray-300 space-y-1">
                  ...
                  <div>P = V × I</div>
                  <div>P = V² / R</div>
                  <div className="mt-2 font-semibold">Resistor Calculation Mode: <span className="text-blue-400">{resistorMode.charAt(0).toUpperCase() + resistorMode.slice(1)}</span></div>
                  {resistorMode === 'series' ? (
            ...
            <div className="bg-black/40 backdrop-blur-lg rounded-2xl border border-white/10 p-6">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full"
                  />
                  <div className="text-blue-400 text-center mt-1">{projectileData.angle}°</div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full"
                  />
                  <div className="text-green-400 text-center mt-1">{projectileData.velocity} m/s</div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full"
                  />
                  <div className="text-purple-400 text-center mt-1">{projectileData.height} m</div>
                </div>
              </div>
              ...
              <div className="flex space-x-2 mt-6">
                ...
                <button
                  ...
                  className="flex-1 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-600 text-white rounded-lg transition-colors flex items-center justify-center space-x-2"
                >
            ...
            <div className="bg-black/40 backdrop-blur-lg rounded-2xl border border-white/10 p-6">
              ...
              <div className="bg-gradient-to-b from-blue-900/20 to-green-900/20 rounded-lg p-4 h-64 relative overflow-hidden">
                <svg width="100%" height="100%" className="absolute inset-0">
                  ...
                  {/* Ground line */}
                  <line x1="0" y1="240" x2="300" y2="240" stroke="#22C55E" strokeWidth="2" />
                </svg>
            ...
            <div className="bg-black/40 backdrop-blur-lg rounded-2xl border border-white/10 p-6">
              ...
              <div className="space-y-4">
                <div className="bg-blue-600/20 rounded-lg p-4 border border-blue-600/30">
                  <div className="text-blue-300 text-sm font-medium mb-1">Max Range</div>
                ...
                <div className="bg-green-600/20 rounded-lg p-4 border border-green-600/30">
                  <div className="text-green-300 text-sm font-medium mb-1">Max Height</div>
                ...
                <div className="bg-purple-600/20 rounded-lg p-4 border border-purple-600/30">
                  <div className="text-purple-300 text-sm font-medium mb-1">Flight Time</div>
