    ...
    <div className="grid lg:grid-cols-3 gap-8">
      ...
      <div className="bg-black/40 backdrop-blur-lg rounded-2xl border border-white/10 p-6">
        <div className="flex items-start justify-between mb-2">
          ...
          <span className="text-xs px-2 py-1 rounded-full bg-blue-600/30 border border-blue-500/40 text-blue-200 whitespace-nowrap">{config.category}</span>
        </div>
        <p className="text-sm text-gray-300 mb-3 leading-relaxed">{config.description}</p>
        {config.formula && (
          <div className="mb-6 text-xs font-mono bg-gray-900/60 border border-white/10 rounded p-3 text-blue-300 overflow-x-auto">
            {config.formula}
          </div>
        )}
          ...
          {config.variables.map(v => (
            <div key={v.key}>
              ...
              <input
                ...
                className="w-full"
              />
              <div className="text-blue-400 text-center mt-1">{vars[v.key]} {v.unit || ''}</div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-black/40 backdrop-blur-lg rounded-2xl border border-white/10 p-6">
        ...
        <div className="space-y-4">
          {config.outputs.map(o => (
            <div key={o.key} className="bg-blue-600/20 rounded-lg p-4 border border-blue-600/30">
              <div className="text-blue-300 text-sm font-medium mb-1">{o.label}</div>
        ...
        <div className="text-sm text-gray-300 space-y-3">
          <div>
            ...
            <ul className="space-y-1">
              {config.variables.map(v => (
                <li key={v.key} className="flex justify-between"><span>{v.label}</span><span className="text-blue-400">{vars[v.key]} {v.unit || ''}</span></li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-1">
              {config.outputs.map(o => (
                <li key={o.key} className="flex justify-between"><span>{o.label}</span><span className="text-green-400">{formatNumber(outputs[o.key])} {o.unit || ''}</span></li>
              ))}
            </ul>
          </div>
